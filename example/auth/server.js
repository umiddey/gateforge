...
const SECRET = process.env.GATEFORGE_AUTH_SECRET ?? 'gateforge-auth-test-secret-v1';
 ...
 */
function createRefund({ id, amountCents, tenantId, requestedBy }) {
  const record = {
    id,
...
async function handle(req, res) {
  ...
  if (getMatch && req.method === 'GET') {
    const refund = findRefund(getMatch[1]);
  ...
  if (path === '/__test/mint' && req.method === 'POST') {
    ...
    const token = mintToken({ sub, role, tenantId, exp });
  ...
  if (path === '/__test/forged' && req.method === 'POST') {
    ...
    const role = body.role === 'tenant' ? 'tenant' : 'admin';
    const tenantId = typeof body.tenantId === 'string' ? body.tenantId : 'tenant-a';
    const token = mintToken({ sub: `${role}-1`, role, tenantId, exp: Math.floor(Date.now() / 1000) + 3600 });
