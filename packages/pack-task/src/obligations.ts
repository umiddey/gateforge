 ...
 */
export const TASK_OBLIGATION_DESCRIPTIONS: Record<TaskObligationContract, string> = {
  'task:retry-policy-enforced':
    'flaky task retries up to maxAttempts before final failure',
  'task:idempotent':
    'execution with the same idempotency key is deduplicated (one side effect)',
  'task:terminal-handled':
    'errors whose type is in terminalOn are NOT retried (terminal outcome)',
  'task:observability-recorded':
    'every execution (success + failure) emits an audit record in runs.json',
  'task:duplicate-delivery-handled':
    'duplicate deliveries of the same task produce exactly one side effect',
};
 ...
 */
export function obligationsFor(resourceId: string): string[] {
