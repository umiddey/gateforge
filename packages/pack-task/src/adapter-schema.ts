...
export type TaskAuditAdapterValidation =
  | { ok: true; adapter: TaskAuditAdapter }
 ...
 */
export function validateTaskAuditAdapter(value: unknown): TaskAuditAdapterValidation {
  ...
  const candidate = value as Partial<TaskAuditAdapter>;
  const issues: string[] = [];
  ...
  return { ok: true, adapter: value as TaskAuditAdapter };
}