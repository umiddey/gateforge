...
export function playwrightVersion() {
    try {
        ...
        return require(pkg).version ?? 'unknown';
    }
    catch {
        return 'unknown';
    }
 ...
 */
export async function executeSupervisedPlaywright(selection, env, options = {}) {
    ...
    if (outcome.error !== null) {
        return incomplete(null, `supervised run could not start the runner (${argv.join(' ')}): ${outcome.error.message}`);
    }
    if (outcome.timedOut) {
        return incomplete(outcome.code, `supervised run exceeded its ${String(options.timeoutMs ?? DEFAULT_RUN_TIMEOUT_MS)}ms bound and was killed — ` +
            'an incomplete run never reports success');
    }
    const document = readOutcomesDocument(outcomesPath);
    if (document === null) {
        return incomplete(outcome.code, 'runner outcomes missing or malformed — the trusted engine reporter writes them on every ' +
    ...
    const fixtureOutcome = document.runStatus === 'passed' && !runnerErrorsFailed
        ? 'passed'
        : document.runStatus === null
            ? 'unknown'
            : 'failed';
    const shardComplete = document.shard === null || document.shard.total <= 1;
    const outcomes = document.outcomes.map((row) => ({
    ...
    const complete = document.runStatus === 'passed' &&
        !runnerErrorsFailed &&
        shardComplete &&
        !retriesDetected &&
        outcome.code === 0;
    return {
        ...
        ...(complete
            ? {}
            : {
                incompleteDetail: document.runStatus !== 'passed'
 ...
 */
export function defaultPlaywrightCommand(cwd) {
...
function readOutcomesDocument(path) {
    ...
    if (typeof document !== 'object' || document === null || Array.isArray(document))
        return null;
    const doc = document;
    if (doc['schemaVersion'] !== 1 || !Array.isArray(doc['outcomes']))
        return null;
    const outcomes = [];
    for (const entry of doc['outcomes']) {
        ...
        outcomes.push({
            testId: row['testId'],
            file: row['file'],
            titlePath: row['titlePath'].filter((s) => typeof s === 'string'),
    ...
    const shard = typeof doc['shard'] === 'object' && doc['shard'] !== null
        ? doc['shard']
        : null;
    return {
        schemaVersion: 1,
        ...
        runnerErrors: Array.isArray(doc['runnerErrors'])
            ? doc['runnerErrors'].filter((s) => typeof s === 'string')
            : [],
        outcomes,
        shard: shard !== null && typeof shard['index'] === 'number' && typeof shard['total'] === 'number'
...
function incomplete(processExit, detail) {
    return {
        processExit,
        complete: false,
        outcomes: [],
