...
import { executeSupervisedPlaywright } from './supervised-run.js';
...
export class PlaywrightAdapter {
     ...
     */
    async resolveInstances(logicalKey, cwd) {
        const config = this.options.config ?? loadConfig(`${cwd}/.gateforge.yml`);
        const { catalog } = await discoverTestCatalog({
            cwd,
            config,
...
export class PytestAdapter {
    ...
    async execute(selection, env) {
        ...
        return {
            ...
            ...(complete
                ? {}
                : {
                    incompleteDetail: result.incompleteDetail ??
     ...
     */
    async executeDiagnostic(repoRoot, stateDir) {
