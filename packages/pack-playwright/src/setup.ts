...
export async function startWitnessProcess(
  ...
  const urlPromise = new Promise<{ url: string; proxyUrl: string | null }>((resolveUrl, rejectUrl) => {
    ...
    child.once('exit', (code) => {
      ...
      if (code !== 0) {
        rejectUrl(
          new Error(`witness child exited early (code ${String(code)}): ${stdout.slice(0, 500)}`),
        );
      }
    });
  });
...
async function waitForHealth(url: string, token: string, timeoutMs: number): Promise<void> {
 ...
 */
export async function gateforgeGlobalSetup(): Promise<void> {
  ...
  if (token === undefined || token === '' || stateDir === undefined || stateDir === '') {
    console.warn(
      '[gateforge] no run state wired (GATEFORGE_STATE_DIR/GATEFORGE_RUN_TOKEN): ' +
...
export async function gateforgeGlobalTeardown(): Promise<void> {
