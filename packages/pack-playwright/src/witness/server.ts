...
import {
  ...
  DOMAIN_CHECK_KINDS,
  DOMAIN_DUAL_SCENARIOS,
  DOMAIN_SCENARIO_CLASSES,
  KNOWN_RECORD_KINDS,
  LOOPBACK_HOSTNAME,
  PERSISTENCE_KIND,
  RUN_HEADER,
...
interface WitnessState {
   ...
   * `http.request` / `<ns>.check` record — they can never fabricate or
   * mutate one.
   */
...
async function handleHttpObservation(
  ...
  const index = state.observed.findIndex(
    (entry) => entry.method === method.toUpperCase() && entry.path === wanted,
  );
  if (index === -1) {
    sendJson(res, 409, {
      error:
        `no engine-observed request matches ${method.toUpperCase()} ${wanted}; drive the ` +
  ...
  const record = issueRecord(
    ...
    'http.request',
    testId,
    { method: observedRequest.method, url: observedRequest.path, status: observedRequest.status },
    'engine-observed',
  );
  sendJson(res, 200, {
...
async function handleDomainCheck(
  ...
  if (
    ...
    sendJson(res, 400, {
      error:
        'domain check requires obligationId, testId, claimId, scenario, and kind one of ' +
  ...
  if (scenarioTable === undefined || outcome === undefined) {
    sendJson(res, 400, {
      error:
        `scenario '${scenario}' is not part of the '${kind}' namespace table; the witness only ` +
  ...
  if (typeof method !== 'string' || method.length === 0 || typeof path !== 'string' || path.length === 0) {
    sendJson(res, 409, {
      error:
        'domain check has no engine-side producer for non-HTTP scenarios yet: this endpoint can ' +
  ...
  const matches: ObservedExchange[] = [];
  ...
  if (!inClass) {
    ...
    sendJson(res, 409, {
      error:
        `engine observed status(s) ${statuses.join(', ')} which cannot evidence scenario ` +
  ...
  const last = matches[matches.length - 1] as ObservedExchange;
...
async function handleRequest(
  ...
  try {
    ...
    if (req.method === 'POST' && path === '/witness/domain-check') {
      await handleDomainCheck(state, res, (await readBody(req)) as Record<string, unknown>);
      return;
    }
