...
export function clearPinnedLoopbackForTests(): void {
  pinnedIps.clear();
}
 ...
 */
export async function pinLoopbackIps(
  hostname: string,
  lookupFn: PinDnsLookup = (host) => lookup(host, { all: true, verbatim: true }),
): Promise<string[]> {
 ...
 */
export function pinnedLoopbackIps(): Map<string, string[]> {
  return new Map([...pinnedIps].map(([host, ips]) => [host, [...ips]] as [string, string[]]));
}
 ...
 */
export function hostResolverRules(pins: ReadonlyMap<string, readonly string[]> = pinnedLoopbackIps()): string | null {
  const rules: string[] = [];
...
function preferIpv4(ips: readonly string[]): string {
  return ips.find((ip) => /^127(\.\d{1,3}){3}$/.test(ip)) ?? (ips[0] as string);
}
  ...
  for (let redirect = 0; redirect <= input.maxRedirects; redirect += 1) {
    ...
    if (response.status >= 300 && response.status < 400 && location !== null) {
      ...
      if (next.protocol !== input.protocol) {
        throw new Error(
          `pinned egress refuses cross-scheme redirect (${input.protocol} → ${next.protocol} '${location}')`,
        );
      }
...
function singleRequest(input: {
  ...
  return new Promise<PinnedGetResult>((resolve, reject) => {
      ...
      (response) => {
        ...
        response.on('end', () => {
          ...
          for (const [name, value] of Object.entries(response.headersDistinct ?? {})) {
            ...
            for (const item of values) headers.append(name, item);
          }
          resolve({
    ...
    if (input.signal !== undefined) {
      if (input.signal.aborted) request.destroy(new Error('pinned egress aborted'));
      else input.signal.addEventListener('abort', () => request.destroy(new Error('pinned egress aborted')), { once: true });
    }
