 ...
 */
export function createWorkflowDetector(options = {}) {
        ...
        async discover(paths) {
            ...
            findings.sort((a, b) => {
                ...
                if (aLoc.file !== bLoc.file)
                    return aLoc.file < bLoc.file ? -1 : 1;
...
// ---------------------------------------------------------------------------
function isCreateMachineCall(expr) {
    if (ts.isIdentifier(expr) && expr.text === 'createMachine')
        return true;
    if (ts.isPropertyAccessExpression(expr) && expr.name.text === 'createMachine')
        return true;
    return false;
}
/**
 ...
 */
function unknownFsmStyleFinding(ctx, call, cause) {
...
function tryExtractXstateMachine(ctx, call) {
    const arg = call.arguments[0];
    if (arg === undefined || !ts.isObjectLiteralExpression(arg)) {
        ctx.onFinding(unknownFsmStyleFinding(ctx, call, 'the config argument is not an object literal'));
        return null;
    }
    const statesProp = arg.properties.find((p) => ts.isPropertyAssignment(p) && ts.isIdentifier(p.name) && p.name.text === 'states');
    if (statesProp === undefined) {
        ctx.onFinding(unknownFsmStyleFinding(ctx, call, 'the config declares no `states` object'));
        return null;
    }
    if (!ts.isObjectLiteralExpression(statesProp.initializer)) {
        ctx.onFinding(unknownFsmStyleFinding(ctx, call, '`states` is not an object literal'));
        return null;
    }
    const machine = newMachine(ctx, 'xstate');
    const stateNames = [];
    for (const stateProp of statesProp.initializer.properties) {
        ...
        if (ts.isObjectLiteralExpression(stateProp.initializer)) {
            const onProp = stateProp.initializer.properties.find((p) => ts.isPropertyAssignment(p) && ts.isIdentifier(p.name) && p.name.text === 'on');
            if (onProp !== undefined && ts.isObjectLiteralExpression(onProp.initializer)) {
                for (const eventProp of onProp.initializer.properties) {
                    if (!ts.isPropertyAssignment(eventProp))
                        continue;
                    const eventName = propertyName(eventProp);
                    if (eventName === null)
                        continue;
                    if (!ts.isObjectLiteralExpression(eventProp.initializer))
                        continue;
                    const targetProp = eventProp.initializer.properties.find((p) => ts.isPropertyAssignment(p) && (ts.isIdentifier(p.name) || ts.isStringLiteral(p.name)) &&
...
function deriveXstateName(ctx, call, stateNames) {
...
// ---------------------------------------------------------------------------
function tryExtractFsmClass(ctx, decl) {
    const className = decl.name?.text;
    if (className === undefined)
        return null;
    if (!/Machine$|FSM$|StateMachine$|Workflow$/.test(className))
        return null;
    const machine = newMachine(ctx, 'fsm');
    ...
    for (const member of decl.members) {
        if (ts.isPropertyDeclaration(member) && ts.isIdentifier(member.name) && member.name.text === 'transitions' &&
            ...
            for (const fromProp of member.initializer.properties) {
                if (!ts.isPropertyAssignment(fromProp))
                    continue;
                const from = propertyName(fromProp);
                if (from === null)
                    continue;
                machine.states.add(from);
                if (!ts.isObjectLiteralExpression(fromProp.initializer))
                    continue;
                const onProp = fromProp.initializer.properties.find((p) => ts.isPropertyAssignment(p) && ts.isIdentifier(p.name) && p.name.text === 'on');
                if (onProp === undefined || !ts.isObjectLiteralExpression(onProp.initializer))
                    continue;
                for (const eventProp of onProp.initializer.properties) {
                    if (!ts.isPropertyAssignment(eventProp))
...
// ---------------------------------------------------------------------------
function tryExtractEnumSwitch(ctx, node) {
    // Detect enum declarations (TS enums).
    if (ts.isEnumDeclaration(node)) {
        const machine = newMachine(ctx, 'enum-switch');
...
function collectEnumSwitchCases(ctx, machine) {
...
// ---------------------------------------------------------------------------
function finalizeMachine(machine, detectorId, detectorVersion) {
...
function dedupTransitions(list) {
    const seen = new Set();
    const out = [];
    for (const t of list) {
