 ...
 */
export function validateEntityAdapter(value: unknown): EntityAdapterValidation {
  ...
  const candidate = value as Partial<EntityAdapter>;
  const issues: string[] = [];
  if (typeof candidate.read !== 'function') issues.push('read: must be a function (ctx, id) => Promise<unknown>');
  if (typeof candidate.normalize !== 'function') issues.push('normalize: must be a function (body) => { entityId, fields }');
  if (issues.length > 0) return { ok: false, issues };
  return { ok: true, adapter: value as EntityAdapter };
}