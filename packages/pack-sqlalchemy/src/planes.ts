 ...
 */
export function fromClassificationsDocument(document: unknown): PlaneRule | null {
  const parsed = ClassificationFileSchema.safeParse(document);
  if (!parsed.success) return null;
  const byName = new Map<string, SqlalchemyPlane | typeof AMBIGUOUS>();
 ...
 */
export function planeRuleFromProjectConfig(cwd: string): PlaneRule {
  const configPath = join(cwd, '.gateforge.yml');
  if (!existsSync(configPath)) return NO_PLANE_MAPPING;
  ...
  if (!existsSync(classificationsPath)) return NO_PLANE_MAPPING;
  try {
