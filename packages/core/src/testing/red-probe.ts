 ...
 */
export async function runRedProbe(probe: RedProbe): Promise<RedProbeRecord> {
  let greenPassed = true;
  let greenError: unknown;
  try {
    await probe.green();
  } catch (error) {
    greenPassed = false;
...
function errorMessage(error: unknown): string {
  if (error === undefined) return '(no error thrown)';
...
export class RedProbeSuiteError extends Error {
  ...
  constructor(records: RedProbeRecord[]) {
    ...
    super(
      `red-probe suite FAILED: ${bad.length} of ${records.length} probe(s) are fake-green or broken guards\n` +
 ...
 */
export function formatProbeRecords(records: RedProbeRecord[]): string {
  const rows = records.map((record) =>
    `| ${record.name} | ${record.greenPassed ? 'yes' : 'NO'} | ${record.brokenFailed ? 'yes' : 'NO'} | ${record.ok ? 'ok' : 'FAKE GREEN / BROKEN GUARD'} |`,
  );
  return [
 ...
 */
export function writeProbeRecords(path: string, records: RedProbeRecord[]): void {
...
let cachedVitestCli: string | undefined;
...
function vitestCliPath(): string {
  if (cachedVitestCli !== undefined) return cachedVitestCli;
 ...
 */
export function spawnVitest(args: string[], options: VitestRunOptions = {}): VitestRunResult {
 ...
 */
export function writeProbeSuite(dir: string, files: Record<string, string>): string {
...
export function resetVitestCliCache(): void {
 ...
 */
export function cleanupProbeSuite(dir: string): void {
