 ...
 */
export function attestationMac(verifierKey: string, body: AttestationBody): string {
  if (typeof verifierKey !== 'string' || verifierKey.length === 0) {
 ...
 */
export function verifyAttestationMac(
  verifierKey: string,
  body: { runId: unknown; invocationId: unknown; inputDigest: unknown; recordIds: unknown },
  mac: unknown,
): boolean {
  if (typeof mac !== 'string' || !MAC_PATTERN.test(mac)) return false;
  if (
    typeof body !== 'object' ||
    body === null ||
    typeof (body as AttestationBody).runId !== 'string' ||
    typeof (body as AttestationBody).invocationId !== 'string' ||
    typeof (body as AttestationBody).inputDigest !== 'string' ||
    !Array.isArray((body as AttestationBody).recordIds)
  ) {
    return false;
  }
  try {
    const expected = Buffer.from(attestationMac(verifierKey, body as AttestationBody), 'hex');
    const claimed = Buffer.from(mac, 'hex');
    return expected.length === claimed.length && timingSafeEqual(expected, claimed);
  } catch {
    return false;
