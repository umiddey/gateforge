 ...
 */
export function trustedPolicyDigest(inputs: readonly TrustedPolicyInput[]): string {
...
export type PolicyWeakeningCheck =
  | { weakened: false; trustedDigest: string; candidateDigest: string }
  | {
 ...
 */
export function policyWeakenedCandidate(
  trustedDigest: string,
  candidateDigest: string,
): PolicyWeakeningCheck {
  ...
  if (trustedDigest !== candidateDigest) {
    return {
      weakened: true,
      reason:
        'candidate policy digest differs from the trusted policy revision: candidate edits to ' +
