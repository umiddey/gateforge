...
export type ContractAvailability =
  | { /** The channel is implemented and its contracts are satisfiable. */
      status: 'available' }
  | { /** Nothing in the namespace is satisfiable; the reason names what is missing. */
      status: 'unavailable';
...
export interface ContractCapability {
   ...
   */
  unavailableContracts: readonly { contract: string; reason: string }[];
  /**
...
const capabilities = new Map<string, ContractCapability>();
 ...
 */
export function registerContractCapabilities(capability: ContractCapability): void {
  if (capabilities.has(capability.namespace)) {
    throw new Error(
      `a capability record for contract namespace '${capability.namespace}' is already registered; ` +
        'registration cannot override another namespace',
    );
  }
 ...
 */
export function capabilityFor(contract: string): ContractCapability | null {
  const separator = contract.indexOf(':');
  const namespace = separator === -1 ? contract : contract.slice(0, separator);
...
export function allCapabilities(): ContractCapability[] {
