  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (segment === '**') {
      if (i === segments.length - 1) {
        ...
        if (i > 0 && !afterGlobStar) regexParts.push('/');
        regexParts.push('.*');
      } else {
    ...
    for (const ch of segment) {
      if (ch === '*') segmentRegex += '[^/]*';
      else if (ch === '?') segmentRegex += '[^/]';
      else segmentRegex += escapeLiteral(ch);
    }
 ...
 */
export function pathInScope(path: string, roots: readonly string[]): boolean {
