/**
 * Classification binding (plan phase 5, ADR 0003 D5): the ONE authoritative
 * path from detector signals to bound effective classifications.
 *
 * `runClassification` runs the deterministic classifier over a built
 * graph's resources, rebinds every decision onto the graph (plane,
 * plane-qualified id, exposure, classification fields, and the sibling
 * `classificationTrace`), and projects every classifier block (plus
 * stale/invalid signals) into policy-engine blocking entries so the gate
 * stays red until each typed reason is resolved. There is no manual
 * classification path beside this — uncertainty resolves toward MORE
 * obligations, and blocks are machine-actionable, never silent.
 *
 * Pure and deterministic: same graph + signals + policy + adapters ⇒
 * byte-identical output. No clock, no network, no filesystem.
 */
...
export interface RunClassificationInput {
  ...
  scan?: {
    ...
    coverage?: Array<{ detector: string; scannedPaths: string[] }>;
...
export function resourceRef(resource: GraphResource): ClassifierResourceRef {
  return {
    name: resource.name,
    id: resource.id,
    kind: resource.kind,
...
function bindingKey(part: {
  ...
  return (
    `${part.name}\u0000${part.kind}\u0000${part.source}\u0000` +
 ...
 */
export function runClassification(input: RunClassificationInput): GraphClassification {
  ...
  const byKey = new Map<string, ClassificationDecision>();
  for (const decision of result.decisions) byKey.set(bindingKey(decision), decision);
  const resources = input.graph.resources.map((resource) =>
    bindDecision(resource, byKey.get(bindingKey(resource))),
  );
 ...
 */
export function classifierBlocking(
