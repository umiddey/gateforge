...
function signalMatchesResource(
  ...
  if (target.resourceName !== undefined && target.resourceName === resource.name) return true;
  if (target.symbol !== undefined) {
    const classQname = resource.attributes['classQname'];
    if (typeof classQname === 'string' && classQname === target.symbol) return true;
    const symbol = resource.attributes['symbol'];
    if (typeof symbol === 'string' && symbol === target.symbol) return true;
  }
  return false;
}
