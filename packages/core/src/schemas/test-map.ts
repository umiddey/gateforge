...
export const TestMapSchema = z
  ...
  .superRefine((document, ctx) => {
    ...
    // never bind the first match.
    const seen = new Map<string, TestMapEntry>();
    for (let index = 0; index < document.tests.length; index += 1) {
      ...
      if (previous !== undefined) {
        ctx.addIssue({
          ...
          message:
            `duplicate sidecar key '${entry.key}' — entries at tests[${String(
              document.tests.indexOf(previous),
            )}] (file ${previous.selector.file}) and tests[${String(index)}] (file ${entry.selector.file})`,
        });
        continue;
      }
