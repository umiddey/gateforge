...
import { GateReceiptSchema } from '../schemas/gate-receipt.js';
 ...
 */
export function gateReceiptMac(verifierKey, body) {
 ...
 */
export function verifyGateReceipt(verifierKey, candidate, expected = {}) {
    ...
    if (!parsed.success) {
        ...
        return {
            ok: false,
            rejection: 'malformed',
            detail: `gate receipt is malformed (expected receiptVersion 1 with the full binding set and mac)` +
    ...
    for (const check of digestChecks) {
        ...
        if (check.receiptValue !== check.expectedValue) {
            return {
                ...
                detail: `gate receipt ${check.label} does not match the current run ` +
    ...
    if (receipt.verdictSummary.blocking !== 0) {
        return {
            ok: false,
            rejection: 'not-clean',
            detail: `gate receipt seals a blocking verdict summary (${String(receipt.verdictSummary.blocking)} blocking); ` +
