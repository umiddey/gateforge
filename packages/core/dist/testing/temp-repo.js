...
export class TempRepo {
    ...
    git(args, { allowFailure = false } = {}) {
        ...
        if (!allowFailure && result.status !== 0) {
            throw new Error(`git ${args.join(' ')} failed (exit ${result.status}) in ${this.root}\n` +
    ...
    commit(message) {
        ...
        if (sha === null)
            throw new Error('commit succeeded but HEAD is unborn');
        return sha;
    }
    /**
 ...
 */
export function withTempRepo(options, body) {
