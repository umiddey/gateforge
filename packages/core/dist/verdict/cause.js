...
const CLEAN_VERDICTS = new Set(['satisfied', 'waived']);
/**
 ...
 */
const EVIDENCE_NOT_COLLECTED_PATTERNS = [
...
export function causeForVerdict(params) {
    if (CLEAN_VERDICTS.has(params.verdict))
 ...
 */
export function capabilityGap(contract) {
    const capability = capabilityFor(contract);
    if (capability === null) {
        return {
            contract,
            observer: 'a registered semantic verifier and its independent observation channel',
            detail: `contract '${contract}' has no registered verifier or capability record: no honest ` +
    ...
    if (capability.availability.status === 'unavailable') {
        return {
            ...
            detail: `contract '${contract}' is unprovable: its namespace '${capability.namespace}' is ` +
    ...
    if (unavailable !== undefined) {
        return {
            contract,
            observer: capability.observer,
            detail: `contract '${contract}' is registered but unavailable: ${unavailable.reason}. ` +
    ...
    if (!capability.contracts.includes(contract)) {
        return {
            contract,
            observer: capability.observer,
            detail: `contract '${contract}' is not implemented by its namespace '${capability.namespace}': ` +
 ...
 */
export function strictCapabilityGaps(obligations) {
    const gaps = [];
    ...
    return gaps.sort((a, b) => a.obligationId < b.obligationId ? -1 : a.obligationId > b.obligationId ? 1 : 0);
}
