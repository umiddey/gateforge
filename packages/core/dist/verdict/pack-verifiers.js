...
import { registerContractCapabilities, registerContractVerifier, } from './registry.js';
 ...
 * record checks run.
 */
const SUPPORTED_HTTP_CONTRACTS = [
 ...
 */
function frontendProofUnavailable(input) {
    return {
        status: 'missing',
        reason: `'${input.obligation.id}': contract 'http:frontend-request-observed' has no independent ` +
 ...
 */
function selectTransportAnchor(input) {
    ...
    const selected = sortedUniqueIds(ids)[0];
 ...
 */
const CONCRETE_HTTP_METHODS = new Set([
    'GET',
    'HEAD',
    'POST',
    'PUT',
 ...
 */
export function interpretObservedPath(rawUrl) {
 ...
 */
function isSupportedRouteShape(canonicalPath) {
 ...
 */
function isConcreteRouteMethod(method) {
 ...
 */
export function resolveHttpRoute(observedMethod, observedPath, candidates, obligationResourceId) {
    for (let index = 0; index < candidates.length; index += 1) {
        ...
        if (candidate === undefined ||
            ...
            return {
                status: 'incomplete',
                reason: `route inventory entry ${index} is not attributable ` +
    ...
    const matched = candidates.filter((candidate) => candidate.method === upperMethod && pathMatchesShape(observedPath, candidate.canonicalPath));
    // Deduplicate repeated facts for the same resource identity only —
    // never merge different resource IDs or planes.
    const seen = {};
    const distinct = [];
    for (const candidate of matched) {
        if (candidate.resourceId in seen)
            continue;
    ...
    if (distinct.length === 0) {
        return {
            status: 'nomatch',
            reason: `observed ${upperMethod} ${observedPath} matches none of the ` +
 ...
 */
function transportInventoryBlock(input) {
    const candidates = input.httpRoutes;
    if (candidates === null || candidates === undefined) {
        return (`'${input.obligation.id}': no route inventory context for ` +
    ...
    for (let index = 0; index < candidates.length; index += 1) {
        ...
        if (candidate === undefined ||
            ...
            !isConcreteRouteMethod(candidate.method) ||
            !isSupportedRouteShape(candidate.canonicalPath)) {
            return (`'${input.obligation.id}': route inventory entry ${index} is not attributable ` +
                `(unknown/dynamic method or unsupported shape); uniqueness cannot be established ` +
 ...
 */
function gradeTransportRecord(record, trust, input, inventoryBlock, candidates) {
    ...
    const resolution = resolveHttpRoute(observedMethod, interpreted.path, candidates, input.obligation.resourceId);
    ...
    if (resolution.status === 'ambiguous') {
        return {
            status: 'invalid',
            reason: `'${input.obligation.id}': ambiguous route attribution: observed ` +
 ...
 *   exchange matching the endpoint shape (plus 2xx for status-ok).
 */
function gradeTransportObservation(input) {
    ...
    if (requests.length === 0) {
        return {
            status: 'missing',
            reason: `'${input.obligation.id}': no '${HTTP_REQUEST_KIND}' record; the witness observed no ` +
    ...
    for (const entry of requests) {
        ...
        if (grade.status === 'satisfied')
            satisfied.push(grade.recordId);
        else if (grade.status === 'invalid')
            invalidReasons.push(grade.reason);
        else
            missingReasons.push(grade.reason);
    }
    if (satisfied.length > 0) {
        const selected = sortedUniqueIds(satisfied)[0];
        return {
            status: 'satisfied',
    ...
    if (invalidReasons.length > 0) {
        const reason = invalidReasons.sort(compareStrings)[0];
        return { status: 'invalid', reason };
    }
    const reason = missingReasons.sort(compareStrings)[0];
...
const HTTP_CAPABILITY = {
    ...
    unavailableContracts: [
        {
            ...
            reason: 'no independent browser/test observation channel exists: the witness observes HTTP ' +
    ...
    observer: 'witness HTTP proxy channel: a witness-observed http.request exchange bound to the run ' +
...
function domainCapability(namespace, channel) {
    return {
        namespace,
        ...
        availability: {
            status: 'unavailable',
            reason: `no engine-owned state-observing producer exists for ${channel}; every contract of ` +
