 ...
 */
function extractRolesFromMiddlewareArgs(args) {
    const roles = new Set();
    let sawAny = false;
...
function joinPath(prefix, suffix) {
    if (prefix === '' && suffix === '')
        return '/';
    if (prefix === '')
        return normalisePath(suffix);
    if (suffix === '')
        return normalisePath(prefix);
 ...
 */
function buildResourceId(method, path) {
    const normalised = path.replace(/^\//, '').replace(/\/$/, '');
    if (normalised === '')
        return `auth.${method.toLowerCase()}.root`;
...
function columnFor(text, offset) {
    let col = 0;
...
function walkFiles(dir) {
    ...
    while (stack.length > 0) {
        ...
        for (const entry of entries) {
            ...
            if (!entry.isFile) {
                if (SKIP_DIR_NAMES[entry.name] === true)
                    continue;
                stack.push(full);
                continue;
            }
 ...
 */
function extractRoleLiterals(args) {
    const out = [];
    const re = /(['"])([^'"]+)\1/g;
    let match;
    let sawAny = false;
 ...
 */
function scanExpress(text, file) {
    const out = [];
...
function scanFastify(text, file) {
    const out = [];
...
function scanFile(text, file) {
    const out = [];
    for (const ep of scanNest(text, file))
        out.push(ep);
    ...
    if (/\bfrom\s+['"]hono['"]/.test(text) || /\brequire\(\s*['"]hono['"]/.test(text)) {
        for (const ep of scanHono(text, file))
            out.push(ep);
    }
    else if (/\bfrom\s+['"]fastify['"]/.test(text) || /\brequire\(\s*['"]fastify['"]/.test(text)) {
        for (const ep of scanFastify(text, file))
            out.push(ep);
    }
    else {
        for (const ep of scanExpress(text, file))
            out.push(ep);
    }
    return out;
}
 ...
 */
export function createAuthDetector(options = {}) {
    ...
    return {
        discover(paths) {
            ...
            for (const file of files) {
                ...
                for (const endpoint of scanFile(text, file))
