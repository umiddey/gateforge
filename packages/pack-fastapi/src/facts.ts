 ...
 */
export function pathDerivedResourceName(rawPath: string): string | null {
...
export function operationForMethod(method: string): 'create' | 'read' | 'update' | 'delete' | null {
  switch (method) {
    case 'POST':
      return 'create';
    case 'GET':
...
export interface CanonicalizationOutcome {
  ...
  unresolved: Array<{ code: string; detail: string; location: Location }>;
...
export function canonicalizeFacts(
  ...
  const keptResources: ContractResource[] = [];
  const unresolved: CanonicalizationOutcome['unresolved'] = [];
  ...
  for (const resource of resources) {
    ...
    const fact: HttpContractFact = {
      schemaVersion: 1,
      role: 'server-route',
      method: method as HttpContractFact['method'],
      ...
      handlerSymbol:
        typeof attributes['handlerSymbol'] === 'string'
          ? (attributes['handlerSymbol'] as string)
          : undefined,
    ...
    if (Array.isArray(requestSchemas)) {
      ...
      if (names.length > 0) fact.requestSchemaSymbols = names;
    }
    facts.push(fact);
    keptResources.push({
      ...resource,
