...
import { loadConfig, verifyGateReceipt } from '@gate-forge/core';
...
export const BROKER_USAGE = 'usage: gateforge broker commit --workspace <dir> --message <msg> [--receipt <path>] [--ref <ref>]\n' +
 ...
 */
function authorityGit(cwd, env, args, extraEnv = {}) {
 ...
 */
function workspaceTreeId(authorityCwd, env, workspace, scratchDir) {
    ...
    if (added.error !== undefined || added.status !== 0) {
        throw new UsageError(`broker: could not snapshot the workspace candidate (git add, exit ${added.status ?? -1}): ` +
    ...
    for (const entry of staged.stdout.split('\0')) {
        ...
        if (mode === '120000' || mode === '160000') {
            throw new BrokerRejection('ENFORCEMENT_UNTRUSTED', `the workspace candidate contains an unsupported entry ('${path}', mode ${mode}); ` +
 ...
 */
async function recomputeWorkspaceDigests(workspace, env) {
 ...
 */
export async function brokerCommitCommand(io, argv) {
    ...
    const receiptPath = typeof options['receipt'] === 'string' && options['receipt'].length > 0
    ...
    const policyGate = evaluateApprovedPolicy(policyResolution, digests.trustedPolicyDigest, digests.config.enforcement?.strictE2E === true);
    if (policyGate.status === 'blocked') {
        throw new BrokerRejection(policyGate.cause, `broker: ${policyGate.detail} (${policyGate.nextAction})`);
    }
    if (policyGate.status === 'unenforced') {
        writeLine(io.stderr, 'broker: no approved policy digest provisioned (GATEFORGE_APPROVED_POLICY_DIGEST); ' +
    ...
    if (!existsSync(receiptPath)) {
        throw new BrokerRejection('RUN_INCOMPLETE', `no gate receipt at '${receiptPath}' — run \`gateforge test-gates --changed\` in the workspace; ` +
    ...
    try {
        ...
        throw new BrokerRejection('ENFORCEMENT_UNTRUSTED', `the gate receipt at '${receiptPath}' is unreadable/malformed: ${error.message}`);
    }
    const verifierKey = io.env['GATEFORGE_WITNESS_VERIFIER_KEY'];
    if (verifierKey === undefined || verifierKey.length === 0) {
        throw new BrokerRejection('ENFORCEMENT_UNTRUSTED', 'no witness verifier key in the broker environment; the receipt cannot be authenticated (fail closed)');
    }
    ...
    if (receipt.invocation !== SUPERVISED_INVOCATION) {
        throw new BrokerRejection('ENFORCEMENT_UNTRUSTED', `the receipt records invocation '${receipt.invocation}', not '${SUPERVISED_INVOCATION}'; ` +
    ...
    const parentSha = expected.status === 0 && /^[0-9a-f]{40}$/.test(expected.stdout.trim()) ? expected.stdout.trim() : null;
    if (receipt.parentSha !== null && receipt.parentSha !== parentSha) {
        throw new BrokerRejection('BROKER_CAS_MISMATCH', `the receipt was sealed against parent '${receipt.parentSha}' but the authoritative ref '${ref}' is at ` +
    ...
    if (updated.status !== 0) {
        throw new BrokerRejection('BROKER_CAS_MISMATCH', `the authoritative ref '${ref}' moved during the commit (compare-and-swap rejected; expected parent ` +
            `'${parentSha ?? 'unborn'}'); no commit was accepted`);
    }
    writeLine(io.stdout, `broker: committed ${newSha} on ${ref} (tree ${treeId}, receipt ${receipt.receiptId}, input ${digests.inputDigest.slice(0, 12)})`);
    writeLine(io.stdout, 'managed-mode note: this guarantee holds only while the broker (Git dir, engine, verifier key) stays outside the agent boundary (ADR 0005 D1)');
    return 0;
}
/**
 ...
 */
export async function brokerCommand(io, argv) {
