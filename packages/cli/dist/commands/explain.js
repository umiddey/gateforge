 ...
 */
export async function explainCommand(io, argv) {
    ...
    const resource = pipeline.graph.resources.find((candidate) => candidate.id === target || candidate.name === target);
    if (resource === undefined) {
        ...
        if (known.length > 0)
            writeLine(io.stderr, `discovered: ${known.join(', ')}`);
        return 1;
    }
    const decision = pipeline.classification.decisions.find((candidate) => candidate.name === resource.name &&
    ...
    const obligations = pipeline.policy.obligations.filter((obligation) => obligation.resourceId === resource.id);
    if (asJson) {
        writeLine(io.stdout, canonicalJson({
            ...
            blocking: pipeline.policy.blocking.filter((entry) => entry.resourceId === resource.id ||
    ...
    if (classification === null || trace === null || decision === undefined) {
        ...
        writeLine(io.stdout, `  lifecycle: create=${String(classification.lifecycle.create)} read=${String(classification.lifecycle.read)} ` +
        ...
        for (const rule of trace.rules)
            writeLine(io.stdout, `    - ${rule}`);
        if (trace.defaultsApplied.length > 0) {
            writeLine(io.stdout, '  conservative defaults applied:');
            for (const rule of trace.defaultsApplied)
                writeLine(io.stdout, `    - ${rule}`);
        }
        writeLine(io.stdout, `  contributing signals: ${String(trace.contributingSignalIds.length)} ` +
    ...
    for (const block of decision?.blocks ?? []) {
        const where = block.locations
            ...
            .join(', ');
        writeLine(io.stdout, `  block [${block.code}] ${block.detail}${where.length > 0 ? ` (${where})` : ''}`);
    }
    writeLine(io.stdout, `obligations (${String(obligations.length)}):`);
    for (const obligation of obligations) {
        writeLine(io.stdout, `  ${obligation.id} (policy ${obligation.policyId}, fingerprint ${fingerprint({
