...
import { loadReceiptFor, receiptGateBlocking, scopedReceiptCoverageBlocking, } from '../receipts.js';
import { resolveProvider } from '../providers.js';
import { computeEvaluationScope, detectStagedWorkingTreeMismatches, } from '../scope.js';
import { freezeStagedCandidate, materializeStagedCandidate, recheckStagedCandidate, releaseStagedCandidate, StagedCandidateBlockError, } from '../staged-candidate.js';
...
export const CHECK_USAGE = 'usage: gateforge check [--changed] [--staged] [--require-e2e] [--format text|json|sarif]\n' +
 ...
 */
async function stagedCheckCommand(io, options) {
    let frozen;
    try {
    ...
    try {
        ...
        if (!recheck.ok) {
            return renderStagedBlock(io, 'ENFORCEMENT_UNTRUSTED', recheck.detail, 'Re-run the gate for the current staged candidate.');
        }
        return code;
    }
    catch (error) {
 ...
 *   number: 1 (blocking).
 */
function renderStagedBlock(io, cause, detail, nextAction) {
 ...
 * @throws fail-closed errors (exit 2) from config/plugin/pipeline layers.
 */
async function runCheckGate(io, options) {
    ...
    const providerIdentity = fixedChangedFiles !== undefined
        ? 'local-staged'
        : diffScoped
    ...
    let scopeDecision = { mode: 'all', changedFiles: [], expandedBecause: [], unmappedFiles: [] };
    let mismatchBlocking = [];
    if (diffScoped) {
        ...
        const knownSourceFiles = [
            ...new Set(pipeline.graph.resources.flatMap((resource) => resource.id === null ? [] : sourcesByResourceId(pipeline.graph).get(resource.id) ?? [])),
        ];
        ...
        const unmappedBlocking = scopeDecision.unmappedFiles
            .filter((file) => !sidecarCoveredFiles.has(file))
            .map((file) => ({
            kind: 'finding',
            resourceId: null,
            name: file,
            detail: `changed file '${file}' has no safe obligation/journey scope — unknown behavior changes ` +
            ...
            if (mismatched.length > 0) {
                ...
                mismatchBlocking = [
                    ...
                    ...mismatched.map((file) => ({
                        ...
                        resourceId: null,
                        name: null,
                        detail: `staged verification cannot certify '${file}': staged (index) bytes differ ` +
    ...
    const coverageChangedFiles = diffScoped && scopeDecision.mode === 'changed' ? scopeDecision.changedFiles : null;
    const requiredCoverage = () => {
        const sources = sourcesByResourceId(pipeline.graph);
        return pipeline.policy.obligations
            .filter((obligation) => {
            if (coverageChangedFiles === null)
                return true;
            const obligationSources = sources.get(obligation.resourceId);
            if (obligationSources === undefined)
                return false;
    ...
    if (requireE2E) {
        if (snapshotUnavailable || expectedDigest === null) {
            receiptBlocking = [
                {
                    ...
                    resourceId: null,
                    name: null,
                    detail: 'require-e2e: the input snapshot is unavailable, so no gate receipt can be validated for this candidate (fail closed)',
                    location: null,
            ...
            if (gate.status === 'blocked') {
                ...
                const load = loadReceiptFor(stateDir, witnessVerifierKey ?? null, {
