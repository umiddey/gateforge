...
export const WAIVE_USAGE = 'usage: gateforge waive <resourceId:contract> --owner <name> --approver <name>\n' +
...
const WAIVE_FLAGS = ['owner', 'approver', 'justification-url', 'expires', 'help'];
/**
 ...
 */
function waiverFilename(resourceId, contract) {
    const sanitize = (value) => value.replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '');
    const name = `${sanitize(resourceId)}-${sanitize(contract)}`;
    if (name === '-' || name.length === 0) {
        throw new UsageError(`waive: cannot derive a waiver filename from '${resourceId}:${contract}' — ` +
 ...
 */
function closeMatches(obligations, resourceId, contract) {
    const sameResource = obligations
        .filter((obligation) => obligation.resourceId === resourceId)
        .map((obligation) => obligation.id);
    const matches = sameResource.length > 0
        ? sameResource
        : obligations
 ...
 */
export async function waiveCommand(io, argv) {
    ...
    if (target === undefined || positionals.length > 1) {
        throw new UsageError(`waive requires exactly one <resourceId:contract> target (${WAIVE_USAGE})`);
    }
    // Obligation-id grammar: split at the FIRST colon — resourceId never
    // contains one, the contract may (`crud:update`).
    const separator = target.indexOf(':');
    if (separator <= 0 || separator === target.length - 1) {
        throw new UsageError(`waive target must be '<resourceId:contract>' (ids split at the first colon), got '${target}' (${WAIVE_USAGE})`);
    }
    ...
    const obligation = pipeline.policy.obligations.find((candidate) => candidate.resourceId === resourceId && candidate.contract === contract);
    if (obligation === undefined) {
        const suggestions = closeMatches(pipeline.policy.obligations, resourceId, contract);
        throw new UsageError(`waive: no obligation resolves for '${resourceId}:${contract}'` +
    ...
    const expiresAt = rawExpires !== undefined && DATE_ONLY.test(rawExpires)
    ...
    const candidate = {
        ...
        approver: approver,
        scope: { kind: 'exact', resourceId, fingerprint },
        expiresAt,
    };
    ...
    if (Date.parse(expiresAt) <= nowMs) {
        throw new UsageError(`waive: --expires '${expiresAt}' is not in the future (injected run clock: ${pipeline.now}) — ` +
    ...
    if (existsSync(targetPath)) {
        ...
        throw new UsageError(`waive: refusing to overwrite existing waiver file '${repoRelative}' — ` +
    ...
    try {
        ...
        throw new UsageError(`waive: rolled back '${repoRelative}' — the waivers directory does not load cleanly: ${detail}`);
    }
    ...
    writeLine(io.stdout, `  expires: ${expiresAt} (in ${remainingDays} day${remainingDays === 1 ? '' : 's'})`);
    ...
    // must not miss it.
    writeLine(io.stdout, 'note: under enforcement.strictE2E this waiver does NOT authorize a change — the waived ' +
