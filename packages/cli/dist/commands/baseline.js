...
export const BASELINE_USAGE = 'usage: gateforge baseline update <fingerprint> [<fingerprint> ...] ' +
 ...
 */
function repeatableFlag(options, name) {
 ...
 */
export function baselineCommand(io, argv) {
    ...
    if (sub !== 'update') {
        throw new UsageError(`unknown baseline subcommand '${sub ?? '<none>'}' (${BASELINE_USAGE})`);
    }
    ...
    const classificationIds = classificationFlagValues.length === 1 && classificationFlagValues[0] === ''
        ? []
    ...
    if (fingerprints.length > 0) {
        ...
        writeLine(io.stdout, `baseline updated: ${next.fingerprints.length} fingerprint(s) (was ${current.fingerprints.length})`);
    }
    if (classificationFlagValues.length > 0) {
        ...
        if (record === null) {
            ...
            throw new UsageError(`no adoption record at ${recordPath} — the classification layer is only ` +
        ...
        writeLine(io.stdout, `classification set updated: ${(next.classificationBlocked ?? []).length} resource(s) remain adopted (was ${(record.classificationBlocked ?? []).length})`);
    }
    return 0;
}
