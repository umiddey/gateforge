...
export async function adoptCommand(io, argv) {
    ...
    for (const entry of evaluated.blocking) {
        ...
        if (identity !== null)
            classificationBlocked.add(identity);
    }
    ...
    writeLine(io.stdout, `classification layer adopted: ${classificationBlocked.size} blocked resource(s) recorded in the receipt ` +
