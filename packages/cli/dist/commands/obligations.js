 ...
 */
export async function obligationsCommand(io, argv) {
    ...
    if (asJson) {
        writeLine(io.stdout, canonicalJson(pipeline.policy));
        return 0;
    }
    ...
    for (const entry of policy.blocking) {
        ...
        writeLine(io.stdout, `  [${entry.kind}] ${entry.resourceId ?? entry.name ?? '<unnamed>'} — ${entry.detail}${where}`);
    }
