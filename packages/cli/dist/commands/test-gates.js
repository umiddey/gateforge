...
import { buildWitnessedPytestChildEnv, discoverTestCatalog, listNativePlaywrightTests, PlaywrightAdapter, readRunnerOutcomes, startSupervisorSpoolDrain, startWitnessProcess, SupervisorClient, TestDiscoveryError, } from '@gate-forge/pack-playwright';
...
import { claimInjectionsFor, planScopedExpectedSet, trustedPolicyDigestForConfig, issueGateReceipt, parentSha, planExpectedSet, sealExecutionResult, supervisionBlocking, SUPERVISED_INVOCATION, } from '../execution.js';
...
import { clearGateReceipt, httpRoutesView, resolveStateDir, stateObligations, writeClaimInjections, writeClassificationsView, writeEnv, writeExecutionResult, writeGateReceipt, writeHttpRoutesView, writeManifest, writeObligations, writeReport, } from '../state.js';
import { loadConfigAt, parseRunFormat, rejectUnknownFlags, VERIFIER_KEY_ENV, VERSION } from './common.js';
export const TEST_GATES_USAGE = 'usage: gateforge test-gates [--changed] [--scope full|changed] [--suite <command>] [--out <dir>] ' +
...
export async function testGatesCommand(io, argv) {
    ...
    rejectUnknownFlags(options, ['suite', 'out', 'format', 'witness-url', 'run-token', 'run-timeout-min', 'changed', 'scope', 'help'], TEST_GATES_USAGE);
    ...
    if (changed && suite !== undefined) {
        throw new UsageError('test-gates: --changed runs the configured suite through the supervised adapter; ' +
 ...
 */
export function parseRunTimeoutMin(raw) {
    if (raw === undefined)
        return undefined;
    if (!/^\d+$/.test(raw)) {
 ...
 * @throws fail-closed errors (exit 2) from config/plugin/pipeline layers.
 */
async function legacyTestGates(io, options) {
    ...
    if (suite !== undefined) {
        ...
        const drain = witnessUrl !== undefined && runToken !== undefined && witnessVerifierKey !== undefined
 ...
 * @throws fail-closed errors (exit 2) from config/plugin/pipeline layers.
 */
async function supervisedTestGates(io, options) {
    ...
    const providerIdentity = options.scope === 'changed'
    ...
    try {
        ...
        discoveryError = error instanceof TestDiscoveryError ? error.message : error.message;
    }
    ...
    const policyGate = evaluateApprovedPolicy(resolveApprovedPolicyDigest({ env: io.env, candidateCwd: io.cwd, candidateConfig: config }), trustedPolicy, config.enforcement?.strictE2E === true);
    ...
    if (catalog !== null) {
        ...
        if (options.scope === 'changed') {
            ...
            scopeBlockers = scopedPlan.unclaimed.map((entry) => ({
                kind: 'finding',
                resourceId: null,
    ...
    const inventoryBlocking = discoveryError !== null
        ? [
            {
                kind: 'finding',
                resourceId: null,
            ...
            ? [
                {
                    ...
                    resourceId: null,
                    name: null,
                    detail: 'test inventory incomplete (parse errors, budget cuts, or unenumerated cases) — the expected set cannot be sealed over an incomplete inventory',
                    location: null,
                    cause: 'TEST_INVENTORY_INCOMPLETE',
                    nextAction: CAUSE_NEXT_ACTIONS.TEST_INVENTORY_INCOMPLETE,
                },
    ...
    const selection = {
        ...
        mode: options.scope === 'changed' ? 'mapped-selection' : 'full-relevant-suite',
        logicalKeys: plannedRows.map((row) => row.planned.logicalKey),
    };
    const selectionDigest = selectionDigestOf(selection);
    const catalogDigest = catalog === null ? NO_CATALOG_DIGEST : sha256Canonical(catalog);
    ...
    if (reuse.reuse) {
        ...
        if ((config.diagnostics?.suites ?? []).some((suite) => suite.witnessed === true)) {
            writeLine(io.stderr, 'witnessed pytest suite(s) sealed in the reused run — identical authenticated inputs, not re-executed');
        }
    ...
    if (options.scope === 'changed' && plannedRows.length === 0) {
        clearGateReceipt(stateDir);
        const emptySliceBlocking = inventoryBlocking.length > 0
        ...
        writeLine(io.stdout, report);
        writeLine(io.stderr, 'test-gates: --scope changed produced no runnable slice — nothing was executed and no receipt was sealed');
        return 1;
    }
    ...
    if (witnessUrl !== undefined) {
        ...
        try {
            ...
            const targetBase = io.env['GATEFORGE_TARGET_BASE_URL'] !== undefined && io.env['GATEFORGE_TARGET_BASE_URL'] !== ''
                ? io.env['GATEFORGE_TARGET_BASE_URL']
                : appBase;
            const adapterBase = io.env['GATEFORGE_ADAPTER_BASE_URL'] !== undefined && io.env['GATEFORGE_ADAPTER_BASE_URL'] !== ''
            ...
            throw new UsageError(`test-gates: the observer (witness service) could not start: ${error.message}`);
        }
    }
    ...
    if (effectiveWitnessUrl === undefined || witnessVerifierKey === undefined) {
        ...
        throw new UsageError('test-gates --changed requires GATEFORGE_WITNESS_VERIFIER_KEY in the orchestrating environment: ' +
    ...
    const plannedIdentities = new Set(plannedRows.map((row) => `${row.planned.project ?? ''}\u0000${row.planned.file}\u0000${row.planned.titlePath.join('>')}`));
    const registeredInstances = options.scope === 'changed'
        ? enumeration.instances.filter((instance) => plannedIdentities.has(`${instance.project ?? ''}\u0000${instance.file}\u0000${instance.titlePath.join('>')}`))
    ...
    writeLine(io.stderr, `expected set registered (${String(registered.count)} test(s), digest ${registered.enumerationDigest.slice(0, 12)}…)`);
    ...
    const adapter = new PlaywrightAdapter({
        ...
        run: {
            ...
            projects: [
                ...new Set(plannedRows.map((row) => row.planned.project).filter((project) => project !== null)),
            ],
    ...
    // path.
    for (const name of ['GATEFORGE_APP_BASE_URL', 'GATEFORGE_TARGET_BASE_URL', 'GATEFORGE_TARGET_FINGERPRINT']) {
    ...
    try {
        ...
        if ((config.diagnostics?.suites ?? []).some((suite) => suite.witnessed === true)) {
            ...
            for (const result of witnessed.results) {
                writeLine(io.stderr, `witnessed pytest ${result.suite}: ${result.status} (passed=${String(result.counts.passed)}` +
            ...
            witnessedBlocking = witnessed.blocking.map((detail) => ({
                kind: 'finding',
                resourceId: null,
                name: null,
                detail,
    ...
    const outcomesDoc = readRunnerOutcomes(join(stateDir, 'runner-outcomes.json'));
    const sealed = sealExecutionResult({
        ...
        ...(options.scope === 'changed' ? { mode: 'mapped-selection' } : {}),
    ...
    const liveAttestation = await fetchWitnessAttestation(io, effectiveWitnessUrl, runToken, witnessVerifierKey, expectedDigest === null
        ? null
        : { runId: manifest.runId, invocationId, inputDigest: expectedDigest });
    if (liveAttestation !== null) {
        persistLiveAttestation(stateDir, liveAttestation);
    ...
    const lifecycleBlocking = lifecycleConflicts.map((detail) => ({
        kind: 'finding',
        resourceId: null,
        name: null,
        detail,
    ...
    const intentBlocking = intentFailures.map((detail) => ({
        kind: 'finding',
        resourceId: null,
        name: null,
        detail,
    ...
    const receipt = issueGateReceipt({
        ...
        ...(options.scope === 'changed'
            ? { scope: 'changed', coveredObligationFingerprints: coveredFingerprints }
            : {}),
        executionResultDigest: sealed.digest,
        evidenceAttestationDigest: liveAttestation === null ? null : sha256Canonical(liveAttestation),
        verdictSummary: {
...
async function runDiagnosticsStep(io, config, cwd, stateDir, inputDigest, now) {
    ...
    for (const result of run.results) {
        const line = `diagnostic ${result.suite}: ${result.status} (passed=${String(result.counts.passed)}` +
            ` failed=${String(result.counts.failed)} errors=${String(result.counts.errors)}` +
            ` skipped=${String(result.counts.skipped)} xfail=${String(result.counts.xfailed)})`;
 ...
 */
async function stopWitnessProcess(handle) {
 ...
 */
async function spawnSuite(io, suite, options) {
