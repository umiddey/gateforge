 ...
 *     hashed as empty).
 */
export function computeTrustedPolicyDigest(cwd, configPaths) {
    const entry = (name, path, required) => {
        ...
        try {
            ...
            throw new UsageError(`cannot read trusted policy document '${path}': ${error.message}`);
        }
    };
    ...
    try {
        ...
        if (error.code !== 'ENOENT') {
            throw new UsageError(`trusted policy digest cannot read the adapters dir '${configPaths.adaptersDir}': ${error.message}`);
        }
 ...
 *   string: 64-char lowercase hex trusted policy digest.
 */
export function trustedPolicyDigestForConfig(cwd, config) {
    const waiverFiles = [];
 ...
 */
export function claimInjectionsFor(resolution, catalog) {
    const catalogByKey = new Map(catalog.entries.map((entry) => [
 ...
 *   logical key.
 */
export function planExpectedSet(catalog) {
    const rows = [];
    for (const entry of catalog.entries) {
        if (entry.runner !== 'playwright')
            continue;
        const blockingAnnotations = [
            ...new Set(entry.suppressionSignals
                .filter((signal) => signal.kind === 'only' || signal.kind === 'skip' || signal.kind === 'fixme')
                .map((signal) => signal.kind)),
        ].sort();
        const unenumerated = entry.discoveryStatus === 'unresolved' && entry.unresolvedReason !== undefined
...
export function planScopedExpectedSet(input) {
    ...
    for (const group of input.resolution.obligations) {
        if (affectedIds.has(group.obligationId))
            continue;
        const declaresChangedClaim = group.bindings.some((binding) => (binding.origin === 'sidecar' || binding.origin === 'native') &&
 ...
 */
export function executedOutcomesOf(outcomesDoc, plannedRows) {
    if (outcomesDoc === null)
        return [];
    const logicalKeyByKey = new Map(plannedRows.map((row) => [
...
function shardCompletenessOf(shards) {
...
export function sealExecutionResult(input) {
    ...
    const supervision = superviseExecution(input.plannedRows.map((row) => row.input), {
    ...
    const draft = ExecutionResultSchema.parse({
        schemaVersion: 1,
        runId: input.runId,
        ...
        catalogDigest: sha256Canonical(input.catalog),
 ...
 */
export function issueGateReceipt(input) {
    ...
    void placeholder;
    const signed = { ...parsed, mac: gateReceiptMac(input.verifierKey, body) };
 ...
 */
export function supervisionBlocking(findings) {
