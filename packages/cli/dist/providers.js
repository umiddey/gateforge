 ...
 */
function gitOutput(cwd, args, context, env) {
    ...
    if (result.status !== 0) {
        ...
        throw new UsageError(`${context}: git ${args.join(' ')} failed (exit ${result.status ?? -1})` +
            (stderr.length > 0 ? `: ${stderr.split('\n')[0] ?? ''}` : ''));
    }
    return (result.stdout ?? '').trim();
}
/** Local staged-diff provider: `git diff --cached --name-only` (pin #5). */
export function localStagedProvider(cwd, env) {
    return {
...
export function githubPrProvider(cwd, env) {
    return {
        provider: 'github-pr',
        changedFiles: () => {
            const baseRef = env['GITHUB_BASE_REF'];
            if (baseRef === undefined || baseRef.length === 0) {
                throw new UsageError('github-pr provider requires GITHUB_BASE_REF (base branch name) in the environment');
            }
            const mergeBase = gitOutput(cwd, ['merge-base', 'HEAD', baseRef], `github-pr provider (merge-base HEAD ${baseRef})`, env);
            const out = gitOutput(cwd, ['diff', '--name-only', mergeBase, 'HEAD'], `github-pr provider (diff ${mergeBase}..HEAD)`, env);
            return normalizeChangedFiles(out.length > 0 ? out.split('\n') : []);
        },
    };
}
/** GitLab merge-request diff provider (`CI_MERGE_REQUEST_DIFF_BASE_SHA`). */
export function gitlabMrProvider(cwd, env) {
    return {
        provider: 'gitlab-mr',
        changedFiles: () => {
            const baseSha = env['CI_MERGE_REQUEST_DIFF_BASE_SHA'];
            if (baseSha === undefined || baseSha.length === 0) {
                throw new UsageError('gitlab-mr provider requires CI_MERGE_REQUEST_DIFF_BASE_SHA in the environment');
            }
            const out = gitOutput(cwd, ['diff', '--name-only', baseSha, 'HEAD'], `gitlab-mr provider (diff ${baseSha}..HEAD)`, env);
            return normalizeChangedFiles(out.length > 0 ? out.split('\n') : []);
        },
    };
}
/** Builds one provider implementation by identity. */
export function providerFor(provider, cwd, env) {
    switch (provider) {
        case 'local-staged':
 ...
 */
export function resolveProvider(configured, cwd, env) {
    let identity;
    if (configured === 'auto') {
        if (env['GITHUB_BASE_REF'] !== undefined && env['GITHUB_BASE_REF'].length > 0) {
            identity = 'github-pr';
        }
        else if (env['CI_MERGE_REQUEST_DIFF_BASE_SHA'] !== undefined &&
