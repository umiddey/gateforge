...
function packageDir(nameOrDetectorId) {
    const short = SHORT_NAMES[nameOrDetectorId] ?? nameOrDetectorId.replace(/^@gateforge\//, '');
    if (!short)
    ...
    for (const candidate of candidates) {
        try {
            ...
            if (manifest.name === packageNameFor(nameOrDetectorId))
                return real;
        }
        catch {
            // probe next
    ...
    throw new UsageError(`bundled detector '${nameOrDetectorId}' (${packageNameFor(nameOrDetectorId)}) is not installed; ` +
        'scan-completeness evidence requires the genuine package');
}
/** The package name that must provide a bundled detector. */
function packageNameFor(nameOrDetectorId) {
 ...
 */
export function validateCoverageTrust(rules, plugins, cwd) {
    assertBundledDetectors(rules.map((rule) => rule.detector), plugins, cwd, 'coverage rule');
}
/**
 ...
 */
export function assertBundledDetectors(detectorIds, plugins, cwd, purpose) {
    for (const detectorId of detectorIds) {
        const packageName = TRUSTED_DETECTOR_PACKAGES[detectorId];
        if (packageName === undefined) {
            throw new UsageError(`${purpose} names detector '${detectorId}', which is not a bundled Gateforge ` +
        ...
        if (plugin === undefined) {
            throw new UsageError(`${purpose} requires detector '${detectorId}' (${packageName}) to be configured ` +
                'in .gateforge.yml');
        }
        ...
        if (plugin.transport === 'in-process') {
            if (plugin.module === undefined) {
                throw new UsageError(`${purpose} requires detector '${detectorId}' to declare its bundled module`);
            }
            let configuredReal;
            try {
                const configured = isAbsolute(plugin.module) || plugin.module.startsWith('./') || plugin.module.startsWith('../')
                ...
                throw new UsageError(`${purpose} requires detector '${detectorId}' to load from its bundled package ` +
            ...
            throw new UsageError(`${purpose} requires detector '${detectorId}' to use the bundled in-process ` +
...
//# sourceMappingURL=detector-trust.js.map