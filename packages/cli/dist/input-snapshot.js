 ...
 */
export function environmentIdentity(parts) {
    const sorted = {};
 ...
 *   usable repository.
 */
function gitNul(cwd, args) {
    const result = spawnSync('git', [...args], { cwd, encoding: 'utf8', maxBuffer: 64 * 1024 * 1024 });
    if (result.error !== undefined || result.status !== 0) {
        throw new SnapshotUnavailableError(`input snapshot requires usable Git inventory (git ${args.join(' ')} failed); ` +
 ...
 */
function hashEntry(kind, path, content) {
 ...
 */
function entryForPath(cwd, path) {
    ...
    try {
        ...
        throw new UnsupportedSnapshotError(`input snapshot cannot inspect '${path}' (${error.code ?? 'UNKNOWN'}); ` +
    ...
    if (stat.isSymbolicLink()) {
        ...
        try {
            ...
            throw new UnsupportedSnapshotError(`input snapshot cannot read symlink '${path}' (${error.message}); ` +
                'explicit unsupported-snapshot block');
        }
        const resolved = resolve(dirname(absolute), target);
        const root = resolve(cwd);
        if (resolved !== root && !resolved.startsWith(root + sep)) {
            throw new UnsupportedSnapshotError(`input snapshot rejects symlink '${path}' escaping the repository; ` +
                'explicit unsupported-snapshot block');
        }
        let targetStat;
        try {
            ...
            throw new UnsupportedSnapshotError(`input snapshot rejects unresolvable symlink '${path}' -> '${target}'; ` +
                'explicit unsupported-snapshot block');
        }
        if (!targetStat.isFile()) {
            throw new UnsupportedSnapshotError(`input snapshot rejects non-file symlink '${path}' -> '${target}'; ` +
                'explicit unsupported-snapshot block');
        }
        let targetBytes;
        try {
            ...
            throw new UnsupportedSnapshotError(`input snapshot cannot read symlink target of '${path}' (${error.message}); ` +
                'explicit unsupported-snapshot block');
        }
    ...
    if (stat.isFile()) {
        ...
        try {
            ...
            throw new UnsupportedSnapshotError(`input snapshot cannot read '${path}' (${error.message}); ` +
                'explicit unsupported-snapshot block');
        }
        return { path, type: 'file', contentDigest: hashEntry('file', path, bytes) };
    }
    throw new UnsupportedSnapshotError(`input snapshot rejects non-file input '${path}'; explicit unsupported-snapshot block`);
}
/**
 ...
 */
export function normalizeRepoModule(module) {
 ...
 */
function collectDeclaredInputs(cwd, config) {
    const paths = new Set();
    // Configured scan inputs even when Git ignores them (the expansion
    // walks the tree directly and never consults .gitignore).
    for (const expanded of expandIncludePaths(config.project.paths.include, config.project.paths.exclude, cwd)) {
    ...
    try {
        ...
        if (!seen)
            paths.add(`absent:${adaptersDir}/(no .mjs adapters)`);
    }
    catch (error) {
        if (error.code === 'ENOENT') {
            ...
            throw new UnsupportedSnapshotError(`input snapshot cannot read adapters dir '${adaptersDir}' (${error.message}); ` +
                'explicit unsupported-snapshot block');
        }
    }
    ...
    try {
        ...
        if (error.code === 'ENOENT') {
            ...
            throw new UnsupportedSnapshotError(`input snapshot cannot read waivers dir '${waiversDir}' (${error.message}); ` +
                'explicit unsupported-snapshot block');
        }
    }
    ...
    for (const plugin of config.plugins) {
        ...
        if (normalized === null) {
            throw new UnsupportedSnapshotError(`input snapshot cannot capture plugin module '${module}' outside the repository; ` +
                'explicit unsupported-snapshot block');
        }
 ...
 */
function collectGitInventory(cwd) {
    const paths = new Set();
    ...
    for (const line of staged) {
        ...
        if (mode === '160000') {
            throw new UnsupportedSnapshotError(`input snapshot cannot capture submodule '${name}'; explicit unsupported-snapshot block`);
        }
        if (name.length > 0) {
 ...
 */
function buildFileEntries(cwd, inventory, tracked, stateDir) {
    const prefix = toPosix(relative(cwd, stateDir));
    const underState = (path) => prefix !== '' && (path === prefix || path.startsWith(`${prefix}/`));
    const entries = [];
 ...
 */
export function assertOutputDisjoint(cwd, stateDir, declaredInputs) {
    const root = resolve(cwd);
    ...
    if (realOut === realRoot) {
        throw new UsageError(`refusing to run: --out '${stateDir}' overlaps the repository root; ` +
    ...
    if (prefix !== '' && !prefix.startsWith('..')) {
        ...
        if (tree.error === undefined && tree.status === 0) {
            ...
            if (committed.length > 0) {
                throw new UsageError(`refusing to run: --out '${stateDir}' overlaps committed source inputs ` +
    ...
    const hidden = declaredInputs
        ...
        .sort(compareStrings);
    if (hidden.length > 0) {
        throw new UsageError(`refusing to run: --out '${stateDir}' overlaps declared source/configuration inputs ` +
 ...
 */
export function buildGateContext(config, plugins = config.plugins.map((plugin) => ({
    id: plugin.id,
    version: plugin.version,
})), classifications = {}, obligations = [], httpRoutes = []) {
 ...
 */
export function digestSnapshot(files, gateContext) {
    return sha256Canonical({
 ...
 */
export function computeInputSnapshot(input) {
    ...
    const gateContext = buildGateContext(input.config, input.plugins, input.classifications ?? {}, input.obligations ?? [], input.httpRoutes ?? []);
    return {
        snapshotVersion: 1,
        files,
        gateContext,
 ...
 */
export function collectInputFiles(cwd, config, stateDir) {
 ...
 */
export function diffInputFiles(before, after) {
    ...
    for (const [path] of afterByPath) {
        if (!beforeByPath.has(path))
            differences.push(`added during discovery: ${path}`);
    }
    return differences.sort(compareStrings);
}
//# sourceMappingURL=input-snapshot.js.map