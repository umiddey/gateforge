        ...
        if (planesConfig !== null && routerFiles.length > 0) {
            ...
            for (const file of routerFiles) {
                ...
                if (resolution.conflict) {
                    ...
                    if (!seenEndpointUnresolved.has(key)) {
                        ...
                        unresolved.push({
                            ...
                            detail: `endpoint '${identity}' (router '${file}') matches ${resolution.hits.length} ` +
                ...
                if (resolution.plane !== null)
                    resolved.push({ file, plane: resolution.plane });
                else
                    uncovered.push(file);
            }
            ...
            if (resolved.length > 1 && distinctPlanes.length > 1) {
                ...
                if (!seenEndpointUnresolved.has(key)) {
                    ...
                    unresolved.push({
                        code: PLANE_RULE_CONTRADICTION,
                        detail: `endpoint '${identity}' is contributed by routes from ${routerFiles.length} router ` +
