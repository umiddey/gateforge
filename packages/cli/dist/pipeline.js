 ...
 */
export function sourcesByResourceId(graph) {
    const map = new Map();
    for (const resource of graph.resources) {
        if (resource.id === null)
            continue;
        const sources = new Set([resource.source]);
...
export async function runPipeline(options) {
    ...
    if (!policyDocParsed.success) {
        throw new UsageError(`classification-policy document is invalid: ${firstIssueText(policyDocParsed.error, 'unknown issue')}`);
    }
    ...
    assertBundledDetectors(policyDocParsed.data.trustedInternalEntryPoints
        .map((entry) => entry.detector)
        .filter((detector) => detector !== undefined), config.plugins, cwd, 'trusted entry point');
    ...
    const { contribution: endpointContribution, inventory: endpointInventory } = compileEndpointContribution(contributions, { cwd });
    ...
    const scannedPaths = [...new Set(coverage.flatMap((entry) => entry.scannedPaths))].sort(compareStrings);
 ...
 */
export function effectiveClassifications(graph, classification) {
    const resources = {};
    const decisions = classification.decisions;
    const decisionFor = (resource) => decisions.find((decision) => decision.name === resource.name &&
    ...
    for (const resource of graph.resources) {
        if (resource.id === null || resource.classification === null)
            continue;
        const decision = decisionFor(resource);
        if (decision?.classification === null || decision === undefined)
            continue;
 ...
 */
function mintDeclarationSignals(cwd, paths, policy, resources) {
    ...
    for (const relPath of paths) {
        ...
        for (let index = 0; index < lines.length; index++) {
            ...
            for (const rule of rules) {
                if (!line.includes(rule.marker))
                    continue;
                const col = Math.max(0, line.indexOf(rule.marker));
                for (const resource of targets) {
                    minted.push(ClassificationSignalSchema.parse({
                        schemaVersion: 1,
