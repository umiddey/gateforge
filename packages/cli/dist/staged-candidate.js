...
export class StagedCandidateBlockError extends Error {
     ...
     *     the cause.
     */
    constructor(causeCode, detail, nextAction = CAUSE_NEXT_ACTIONS[causeCode]) {
        super(detail);
...
function git(cwd, env, call) {
    ...
    if (result.status !== 0 && !call.allowFailure) {
        throw new UsageError(`staged candidate: git ${call.args.join(' ')} failed (exit ${result.status ?? -1})` +
 ...
 */
function resolveSha(cwd, env, ref) {
...
function parseNameStatus(raw) {
    ...
    while (index < parts.length) {
        ...
        if (status === 'R' || status === 'C') {
            ...
            index += 1;
            if (src === undefined || src.length === 0 || dst === undefined || dst.length === 0)
                continue;
 ...
 */
function writeTreeOfIndexCopy(cwd, env, indexPath) {
    ...
    if (out.status !== 0) {
        ...
        if (unmerged.stdout.trim().length > 0) {
            throw new StagedCandidateBlockError('ENFORCEMENT_UNTRUSTED', 'the staged index has unmerged conflict entries; resolve the conflicts and restage — ' +
                'a candidate with conflicts cannot be verified', 'Resolve the merge conflicts, restage, and re-run the gate.');
        }
 ...
 */
function copyIndex(cwd, env, scratchDir) {
 ...
 */
function assertSupportedTree(cwd, env, treeId) {
    ...
    for (const entry of out.stdout.split('\0')) {
        ...
        if (mode === '120000') {
            throw new StagedCandidateBlockError('ENFORCEMENT_UNTRUSTED', `the staged candidate contains symlink '${path}'; symlink candidates are not verifiable yet ` +
                '(explicit block until implemented — never a fallback)', 'Replace the staged symlink with a regular file, or run the gate without --staged after implementing symlink support.');
        }
        if (mode === '160000') {
            throw new StagedCandidateBlockError('ENFORCEMENT_UNTRUSTED', `the staged candidate contains submodule '${path}'; submodule candidates are not verifiable yet ` +
                '(explicit block until implemented — never a fallback)', 'Remove the staged submodule, or run the gate without --staged after implementing submodule support.');
        }
    }
}
/**
 ...
 */
export function freezeStagedCandidate(cwd, env) {
    ...
    try {
        ...
        const changed = parseNameStatus(git(cwd, env, {
            args: ['diff-index', '--cached', '--name-status', '-z', '-M', base],
            extraEnv: { GIT_INDEX_FILE: indexPath },
        }).stdout);
        const changedPaths = normalizeChangedFiles(changed.flatMap((change) => (change.oldPath !== undefined ? [change.oldPath, change.path] : [change.path])));
        return {
            treeId,
            headSha,
            mergeHeadSha,
            parentShas: mergeHeadSha !== null ? [headSha, mergeHeadSha] : headSha !== null ? [headSha] : [],
            changed,
            changedPaths,
            scratchDir,
            checkoutDir: null,
 ...
 */
export function materializeStagedCandidate(cwd, env, frozen) {
    ...
    for (const args of [['init', '-q'], ['add', '-A']]) {
        ...
        if (result.error !== undefined || result.status !== 0) {
            throw new UsageError(`staged candidate: preparing the isolated checkout failed (git ${args.join(' ')}, ` +
 ...
 */
export function recheckStagedCandidate(cwd, env, frozen) {
    try {
        ...
        if (treeNow !== frozen.treeId || headNow !== frozen.headSha || mergeNow !== frozen.mergeHeadSha) {
            return {
                ok: false,
                detail: 'the staged candidate changed during verification; re-run the gate ' +
        ...
        return { ok: false, detail: `the staged candidate could not be re-checked: ${error.message}` };
    }
}
/**
 ...
 */
export function releaseStagedCandidate(frozen) {
 ...
 */
export function resolveHooksDir(cwd, env) {
 ...
 */
export function gateforgeHookScript() {
    return `\
${HOOK_MARKER_BEGIN}
...
export function hasGateforgeMarker(body) {
 ...
 */
export function verifyHookActivation(hookPath) {
    let body;
    try {
        body = readFileSync(hookPath, 'utf8');
    }
    catch (error) {
        return { ok: false, detail: `hook file '${hookPath}' is unreadable: ${error.message}` };
    }
    ...
    try {
        ...
        return { ok: false, detail: `hook '${hookPath}' could not be stat-ed: ${error.message}` };
    }
    const run = spawnSync(hookPath, [HOOK_VERIFY_ARG], { encoding: 'utf8', timeout: 10_000 });
    if (run.error !== undefined || run.status !== 0) {
        return {
            ok: false,
            detail: `hook '${hookPath}' did not execute its ${HOOK_VERIFY_ARG} check ` +
...
//# sourceMappingURL=staged-candidate.js.map