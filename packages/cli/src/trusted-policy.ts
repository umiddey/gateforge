...
import { loadConfig, policyWeakenedCandidate, type CauseCode, type GateforgeConfig } from '@gate-forge/core';
 ...
 */
export const WEAKENED_POLICY_NEXT_ACTION =
  'the candidate changes owner-owned policy (classifiers/exclusions/waivers/baselines/coverage); ' +
...
export const PROVISION_PIN_NEXT_ACTION =
  'provision the owner-approved policy digest OUTSIDE the candidate ' +
...
export type ApprovedPolicyResolution =
  | { status: 'ok'; digest: string | null; origins: ApprovedPolicyOrigin[] }
  | { status: 'candidate-controlled'; detail: string }
...
export type ApprovedPolicyGate =
  | { status: 'enforced'; approved: string }
  | { status: 'unenforced' }
  | { status: 'blocked'; cause: CauseCode; detail: string; nextAction: string };

/** The result of {@link assertApprovedPolicy} / {@link assertReceiptApprovedPolicy}. */
export type ApprovedPolicyResult =
  | { ok: true; enforced: true; approved: string }
  | { ok: true; enforced: false }
  | { ok: false; cause: CauseCode; detail: string; nextAction: string };
...
export function resolveApprovedPolicyDigest(options: {
  ...
  if (candidateDeclared !== undefined) {
    return {
      ...
      detail:
        `the candidate's own .gateforge.yml declares enforcement.approvedPolicyDigest; ` +
        'candidate-controlled files cannot approve policy (remove the key and provision the digest outside the candidate)',
    };
  }
  const sources: Array<{ origin: ApprovedPolicyOrigin; digest: string }> = [];
  ...
  if (envDigest !== undefined && envDigest.length > 0) {
    if (!DIGEST_PATTERN.test(envDigest)) {
      return {
        status: 'invalid',
        detail:
          `${APPROVED_POLICY_DIGEST_ENV} must be 64-char lowercase hex (got '${envDigest}'); ` +
  ...
  if (trustedConfigPath !== undefined && trustedConfigPath.length > 0) {
    ...
    if (isInside(resolvedTrusted, resolve(options.candidateCwd))) {
      return {
        status: 'candidate-controlled',
        detail:
          `${TRUSTED_CONFIG_ENV} points at '${trustedConfigPath}', which is inside the candidate repository; ` +
    ...
    try {
      ...
      return {
        status: 'invalid',
        detail:
          `${TRUSTED_CONFIG_ENV} file '${trustedConfigPath}' is not a valid gateforge config: ` +
 ...
 */
export function assertApprovedPolicy(candidateDigest: string, input: ApprovedPolicyInput): ApprovedPolicyResult {
  if (input.approved === null) {
    if (!input.strict) return { ok: true, enforced: false };
    return {
      ok: false,
 ...
 */
export function evaluateApprovedPolicy(
  resolution: ApprovedPolicyResolution,
  candidateDigest: string,
  strict: boolean,
): ApprovedPolicyGate {
  if (resolution.status !== 'ok') {
    if (!strict) return { status: 'unenforced' };
    return {
      status: 'blocked',
      cause: 'ENFORCEMENT_UNTRUSTED',
      detail: resolution.detail,
      nextAction:
        resolution.status === 'candidate-controlled'
 ...
 */
export function assertReceiptApprovedPolicy(
  receipt: { approvedPolicyDigest?: string },
  approved: string,
  ...
  if (sealed === undefined) {
    return {
      ...
      cause: 'ENFORCEMENT_UNTRUSTED',
      detail:
        'the gate receipt carries no approvedPolicyDigest binding, so it cannot prove it was sealed under the ' +
  ...
  if (sealed !== approved) {
    return {
      ...
      cause: 'ENFORCEMENT_UNTRUSTED',
      detail:
        `the gate receipt was sealed under approved policy '${sealed}' but the currently approved revision is ` +
...
export const RESEAL_NEXT_ACTION =
  'rerun `gateforge test-gates --changed` with the currently approved policy digest provisioned to seal a fresh receipt';
 ...
 */
export function describeApprovedPolicyResolution(resolution: ApprovedPolicyResolution): string {
  ...
  if (resolution.digest === null) {
    return (
      'approved policy digest: absent (policy-revision ownership NOT enforced — provision ' +
