...
export type ReceiptLoad =
  | { status: 'ok'; receipt: GateReceipt; executionResult: ExecutionResult }
  | { status: 'absent' }
...
export function loadReceiptFor(
  ...
  const executionResult = executionParsed.data as ExecutionResult;
  if (executionResultDigestOf(executionResult) !== verified.receipt.executionResultDigest) {
    return {
      status: 'execution-mismatch',
      detail:
        'the execution result no longer matches the receipt digest — the supervision record was altered (fail closed)',
    };
  }
