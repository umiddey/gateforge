...
export function gateforgeHookScript(
  gateArgs: readonly string[] = HOOK_GATE_COMMAND.split(' '),
): string {
  const gateCommand = gateArgs.map((argument) => `'${argument.replaceAll("'", "'\\''")}'`).join(' ');
  return `\
${HOOK_MARKER_BEGIN}
# Generated by \`gateforge init --blocking\` (ADR 0005 D1/D3).
# Gates the EXACT staged candidate — never the working tree:
#   gateforge ${gateArgs.join(' ')}
...
case "$GF_BIN" in
  */*) exec node "$GF_BIN" ${gateCommand} ;;
  *)   exec "$GF_BIN" ${gateCommand} ;;
esac
${HOOK_MARKER_END}
`;
}
