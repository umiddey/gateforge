 ...
 */
function isMasterTree(relativeDir: string): boolean {
 ...
 */
export function inferPlanesConfig(tableSources: readonly string[]): PlaneInference {
  ...
  if (business.length === 0) {
    return {
      config: null,
      note:
        'every discovered table lives under a test directory — no business plane to propose ' +
  ...
  for (const head of [...groups.keys()].sort()) {
    ...
    const plane: SqlalchemyPlane = isMasterTree(head === '' ? prefixPath : head) ? 'master' : 'tenant';
    rules.push({
      match,
      plane,
      reason:
        `inferred from model directory '${treePath}' by gateforge init; ` +
