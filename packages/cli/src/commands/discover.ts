 ...
 */
import { canonicalJson, type JsonValue } from '@gateforge/core';
 ...
 */
export async function discoverCommand(io: Io, argv: readonly string[]): Promise<number> {
  ...
  for (const resource of graph.resources) {
    ...
    writeLine(
      io.stdout,
      `  ${id} [${resource.kind}] ${resource.source}:${resource.location.line}` +
