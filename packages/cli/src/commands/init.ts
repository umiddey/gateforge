...
function configTemplate(languages: readonly string[], options: { strictE2E?: boolean } = {}): string {
...
# strict staged gate reports unresolved findings, unwaived obligations,
# or missing proof. Gates the EXACT staged candidate (\`check --staged\`
# freezes the index and verifies the staged bytes, never the worktree).
# Engine resolution: repo-local node_modules/.bin, then PATH, then
# $GATEFORGE_CLI. A missing engine blocks (fail closed).
set -e
GF_BIN=""
...
if [ -z "$GF_BIN" ]; then
  echo "gateforge: CLI engine not found (install gateforge or set GATEFORGE_CLI)" >&2
  exit 1
fi
case "$GF_BIN" in
  */*) exec node "$GF_BIN" check --staged --require-e2e ;;
  *)   exec "$GF_BIN" check --staged --require-e2e ;;
esac
`;
...
function appendPreCommitHook(io: Io): void {
