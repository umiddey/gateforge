...
function collectFiles(input: string): string[] {
  const stat = (() => {
    try { return require('node:fs').statSync(input); } catch { return null; }
  })();
  if (stat === null) return [];
  if (stat.isDirectory()) {
    const { readdirSync } = require('node:fs') as typeof import('node:fs');
    return readdirSync(input)
      .filter((n: string) => /\.(ts|tsx|js|mjs)$/.test(n))
      .map((n: string) => resolve(input, n));
  }
  return [input];
}

/** Detects a `z.object({...})` schema declaration. */
function detectZod(text: string, file: string): DetectedSchema[] {
  ...
  while ((m = decl.exec(text)) !== null) {
    ...
    out.push({
      id: `validation.zod.${name.toLowerCase()}` as ValidationResourceId,
      library: 'zod',
      name,
      boundary: 'strict',
...
function extractZodFields(text: string, openIdx: number): Record<string, { type: string; constraints: string[] }> {
  const out: Record<string, { type: string; constraints: string[] }> = {};
  let depth = 1;
  let i = openIdx;
...
function detectJoi(text: string, file: string): DetectedSchema[] {
  const out: DetectedSchema[] = [];
  ...
  while ((m = decl.exec(text)) !== null) {
    ...
    const line = text.substring(0, m.index).split('\n').length;
    out.push({
      id: `validation.joi.${name.toLowerCase()}` as ValidationResourceId,
      library: 'joi',
      name,
      boundary: 'strict',
      fields: {},
...
function detectYup(text: string, file: string): DetectedSchema[] {
  const out: DetectedSchema[] = [];
  ...
  while ((m = decl.exec(text)) !== null) {
    ...
    const line = text.substring(0, m.index).split('\n').length;
    out.push({
      id: `validation.yup.${name.toLowerCase()}` as ValidationResourceId,
      library: 'yup',
      name,
      boundary: 'lenient',
      fields: {},
...
function detectClassValidator(text: string, file: string): DetectedSchema[] {
  const out: DetectedSchema[] = [];
  if (!/from\s+['"]class-validator['"]/.test(text)) return out;
  ...
  while ((m = clsRe.exec(text)) !== null) {
    ...
    const line = text.substring(0, m.index).split('\n').length;
    out.push({
      id: `validation.class-validator.${name.toLowerCase()}` as ValidationResourceId,
...
function toResource(root: string, s: DetectedSchema): Resource {
