...
const SKIP_DIRS = { 'node_modules': true, 'dist': true, '.git': true };
...
function toRepoRelative(root, absFile) {
...
function inferAlgorithm(hint) {
    ...
    if (lower.includes('sha256'))
        return 'hmac-sha256';
    if (lower.includes('none') || lower.includes('unsigned'))
        return 'none';
    return 'hmac-sha256';
}
/** Signature-header inference from adjacent text. */
function inferSignatureHeader(text) {
    const patterns = [
        /signatureHeader\s*[:=]\s*['"]([^'"]+)['"]/,
        /signature\s*[:=]\s*['"]([^'"]+)['"]/,
        /headers\s*\[\s*['"]([^'"]+)['"]\s*\]/,
...
function finding(code, detail, file, line, col) {
    return {
        code,
        detail,
...
function scanRouteRegistries(text, file, options) {
    const out = [];
...
function scanDecorators(text, file, options) {
    const out = [];
    ...
    while ((m = re.exec(text)) !== null) {
        ...
        const literal = m[2] ?? '';
        if (decoratorName === 'on' && !/^webhook[.:]/i.test(literal))
            continue;
...
export function discoverWebhooks(paths, options = {}) {
    ...
    for (const d of detected) {
        ...
        if (existing !== undefined) {
            findings.push(finding('DUPLICATE_RESOURCE_ID', `duplicate webhook id ${id} (also at ${existing})`, d.sourceFile, d.sourceLine, d.sourceCol));
            continue;
        }
...
export function createWebhookDetector(options = {}) {
    return {
        discover(paths) {
