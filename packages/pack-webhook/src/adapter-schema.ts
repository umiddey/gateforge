...
export type WebhookEntityAdapterValidation =
  | { ok: true; adapter: WebhookEntityAdapter }
 ...
 */
export function validateWebhookEntityAdapter(value: unknown): WebhookEntityAdapterValidation {
  ...
  const candidate = value as Partial<WebhookEntityAdapter>;
  const issues: string[] = [];
  ...
  return { ok: true, adapter: value as WebhookEntityAdapter };
}